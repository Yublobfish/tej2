...
var Attributes2 = class extends Attributes {
  ...
  next() {
    ...
    for (const [_, attrs2] of this.updates) {
      for (const [_2, attr] of attrs2) {
...
var Globals = class {
  constructor(globals) {
    this.attrs = /* @__PURE__ */ new Map();
    this.updates = /* @__PURE__ */ new Map();
    this.self = new import_rxjs6.BehaviorSubject(this);
    globals.subscribe({
      next: ({ attribute, done }) => {
        if (attribute) {
          let val = void 0;
  ...
  obs(key) {
    ...
    if (!o) {
      o = new import_rxjs6.BehaviorSubject(void 0);
...
var Scopes2 = class extends Scopes {
  constructor(scopesObs, donesObs, ctx, kinds2, attributes, taj) {
...
var Subscriptions = class {
  ...
  newSubs() {
    ...
    const {
      scopes: { ids, kinds: kinds2, names, keys, kvs },
      participants,
      transitions
    } = this.last;
    ...
    const next = {
      ...
      scopes: {
        ...
        kinds: current.scopes.kinds.filter((kind) => !kinds2.includes(kind)),
...
// src/admin/runloop.ts
var Runloop = class {
  constructor(conn, ctx, kinds2, globalScopeID, subs, stop) {
    this.conn = conn;
    this.ctx = ctx;
    this.kinds = kinds2;
  ...
  async postCallback(final) {
    ...
    if (this.attributeInputs.length > 0) {
      ...
      const attrs2 = Object.values(uniqueAttrs);
      promises.push(this.taj.setAttributes(attrs2));
      this.attributeInputs = [];
    }
  ...
  loadAllScopes(filters, after) {
    this.taj.scopes({ filter: filters, first: 100, after }).then((conn) => {
      const scopes = {};
      for (const edge of (conn == null ? void 0 : conn.edges) || []) {
      ...
      if ((conn == null ? void 0 : conn.pageInfo.hasNextPage) && (conn == null ? void 0 : conn.pageInfo.endCursor)) {
        return this.loadAllScopes(filters, conn == null ? void 0 : conn.pageInfo.endCursor);
      }
    });
  }
...
var TokenProvider = class {
  constructor(taj, storage2, serviceName, serviceRegistrationToken) {
    this.tokens = bsu(void 0);
    let connected = false;
    let token;
    this.sub = subscribeAsync(
      (0, import_rxjs10.merge)(taj.connected, storage2.tokens),
      async (tokenOrConnected) => {
        if (typeof tokenOrConnected === "boolean") {
          connected = tokenOrConnected;
        } else {
...
var AdminContext = class {
  ...
  static async init(url, tokenFile, serviceName, serviceRegistrationToken, ctx, kinds2) {
    const adminContext = new this(url, ctx, kinds2);
  ...
  async stop() {
    ...
    await this.stopSubs();
    this.tajriba.stop();
    (_b = this.adminConn) == null ? void 0 : _b.stop();
  }
  register(subscriber) {
  ...
  async initSubs() {
    ...
    try {
      ...
      globalScopeID = (_a = scopes.edges[0]) == null ? void 0 : _a.node.id;
      if (!globalScopeID) {
...
function scopeConstructor(input) {
  return [
    input,
    {
      get(key) {
        var _a;
        const attr = (_a = input.attributes) == null ? void 0 : _a.find((a) => a.key === key);
        if (!attr) {
          return;
        }
...
var Game = class extends BatchOwned {
  ...
  get currentRound() {
    var _a;
    return (_a = this.currentStage) == null ? void 0 : _a.round;
  }
  get hasEnded() {
    return endedStatuses.includes(this.get("status"));
  }
  ...
  async assignPlayer(player) {
    ...
    if (previousGameID) {
      await ((_a = this.scopeByID(previousGameID)) == null ? void 0 : _a.removePlayer(player));
    }
  ...
  addRound(attributes) {
    ...
    const addStage = (attributes2) => {
      ...
      const res = import_zod.z.number().int().gte(1).safeParse(durAttr == null ? void 0 : durAttr.value);
      if (!res.success) {
...
var Player = class extends GameOwned {
  ...
  get currentStage() {
    var _a;
    return (_a = this.currentGame) == null ? void 0 : _a.currentStage;
  }
  get stage() {
    const stage = this.currentStage;
    if (!stage) {
...
var Round = class extends GameOwned {
  ...
  addStage(attributes) {
    ...
    const res = import_zod.z.number().int().gte(5).safeParse(durAttr == null ? void 0 : durAttr.value);
    if (!res.success) {
      throw new Error(`stage duration invalid: ${res.error}`);
    }
    const duration = res.data;
...
function Classic({
  ...
  return function(_) {
    ...
    async function assignplayer(ctx, player, skipGameIDs) {
      ...
      for (const batch of evt(ctx).batches) {
        ...
        for (const game2 of batch.games) {
          if (!game2.hasStarted && (!skipGameIDs || !(skipGameIDs == null ? void 0 : skipGameIDs.includes(game2.id)))) {
    ...
    _.on("stage", "gameID", async (ctx, { stage }) => {
      var _a;
      if (!((_a = stage.currentGame) == null ? void 0 : _a.isRunning)) {
        return;
      }
    ...
    _.after(
      ...
      (ctx, { playerStage, submit }) => {
        ...
        const haveAllPlayersSubmitted = players.every((p) => {
          var _a;
          return p.stage.get("submit") || p.get("ended") || !online.has((_a = p.get("participantID")) == null ? void 0 : _a.toString());
        });
...
var Step2 = class {
  constructor(step, ticker) {
    this.running = false;
    this.ticker = new import_rxjs12.BehaviorSubject(void 0);
    this.startAt = 0;
    this.endAt = 0;
    ticker.pipe((0, import_rxjs12.map)(this.recalc.bind(this))).subscribe({
      next: (val) => {
...
var Steps = class {
  constructor(stepsObs, donesObs) {
    ...
    this.ticker = new import_rxjs12.BehaviorSubject(Math.floor(pnow()));
...
function EmpiricaClassic(participantID, provider) {
  const attributesDones = new import_rxjs13.Subject();
  const scopesDones = new import_rxjs13.Subject();
  ...
  const ret = {
    game: new import_rxjs13.BehaviorSubject(void 0),
    player: new import_rxjs13.BehaviorSubject(void 0),
    players: new import_rxjs13.BehaviorSubject([]),
    round: new import_rxjs13.BehaviorSubject(void 0),
    stage: new import_rxjs13.BehaviorSubject(void 0),
    globals: glob.self
  };
...
var ParticipantConnection = class {
  constructor(taj, sessions, resetSession) {
    ...
    this._sessionsSub = subscribeAsync(
      (0, import_rxjs14.merge)(taj.connected, sessions),
...
var TajribaProvider = class {
  constructor(changes, globals, setAttributes) {
    this.globals = globals;
    this.setAttributes = setAttributes;
    this.scopes = new import_rxjs15.Subject();
    this.attributes = new import_rxjs15.Subject();
    this.participants = new import_rxjs15.Subject();
    this.steps = new import_rxjs15.Subject();
    this.dones = new import_rxjs15.Subject();
    changes.pipe((0, import_rxjs15.groupBy)((chg) => chg.change.__typename)).subscribe({
      next: (group) => {
        switch (group.key) {
          case "ScopeChange":
            group.subscribe({
...
function Logo() {
  return /* @__PURE__ */ import_react15.default.createElement(
    ...
    /* @__PURE__ */ import_react15.default.createElement("path", { d: "M25.164 81.9737C22.31 81.9737 19.998 84.2877 20 87.1417L20.028 128.458C20.03 131.309 22.341 133.619 25.192 133.619C28.046 133.619 30.359 131.304 30.357 128.451L30.328 87.1347C30.326 84.2837 28.015 81.9737 25.164 81.9737Z" }),
    /* @__PURE__ */ import_react15.default.createElement("path", { d: "M87.1367 61.3158C84.2837 61.3158 81.9707 63.6298 81.9727 66.4828L82.0017 118.128C82.0037 120.979 84.3147 123.29 87.1667 123.29C90.0197 123.29 92.3327 120.976 92.3307 118.122L92.3007 66.4778C92.2997 63.6258 89.9877 61.3158 87.1367 61.3158Z" }),
    /* @__PURE__ */ import_react15.default.createElement("path", { d: "M107.793 20C104.94 20 102.628 22.313 102.629 25.166L102.661 159.442C102.662 162.294 104.974 164.605 107.826 164.605C110.679 164.605 112.991 162.293 112.99 159.44L112.957 25.163C112.957 22.311 110.645 20 107.793 20Z" }),
    /* @__PURE__ */ import_react15.default.createElement("path", { d: "M128.451 20C125.598 20 123.286 22.313 123.287 25.166L123.319 159.442C123.32 162.294 125.632 164.605 128.484 164.605C131.337 164.605 133.649 162.293 133.648 159.44L133.615 25.163C133.615 22.311 131.303 20 128.451 20Z" }),
    /* @__PURE__ */ import_react15.default.createElement("path", { d: "M149.11 20C146.257 20 143.944 22.313 143.946 25.167L143.977 102.634C143.978 105.485 146.29 107.796 149.141 107.796C151.994 107.796 154.307 105.483 154.306 102.63L154.274 25.162C154.273 22.311 151.961 20 149.11 20Z" }),
    /* @__PURE__ */ import_react15.default.createElement("path", { d: "M169.75 40.6579C166.897 40.6579 164.585 42.9709 164.586 45.8239L164.617 138.785C164.618 141.637 166.93 143.948 169.782 143.948C172.635 143.948 174.947 141.634 174.946 138.781L174.914 45.8209C174.914 42.9689 172.602 40.6579 169.75 40.6579Z" }),
    /* @__PURE__ */ import_react15.default.createElement("path", { d: "M45.8203 61.3158C42.9673 61.3158 40.6553 63.6288 40.6563 66.4818L40.6873 159.443C40.6883 162.295 43.0003 164.606 45.8523 164.606C48.7053 164.606 51.0173 162.292 51.0163 159.439L50.9843 66.4788C50.9843 63.6268 48.6723 61.3158 45.8203 61.3158Z" }),
    /* @__PURE__ */ import_react15.default.createElement("path", { d: "M66.4785 61.3158C63.6255 61.3158 61.3135 63.6288 61.3145 66.4818L61.3455 159.443C61.3465 162.295 63.6585 164.606 66.5105 164.606C69.3635 164.606 71.6755 162.292 71.6745 159.439L71.6425 66.4788C71.6415 63.6268 69.3305 61.3158 66.4785 61.3158Z" })
  );
}
...
function EmpiricaMenu() {
  ...
  return /* @__PURE__ */ import_react16.default.createElement("div", { className: "group fixed top-full left-full -mt-20 -ml-20 rounded-lg bg-white z-20" }, /* @__PURE__ */ import_react16.default.createElement("div", { className: "w-14 h-14 p-2  text-empirica-500 shadow rounded-lg group-hover:shadow-none" }, /* @__PURE__ */ import_react16.default.createElement(Logo, null)), /* @__PURE__ */ import_react16.default.createElement("div", { className: "hidden group-hover:block absolute rounded-lg overflow-hidden bottom-0 right-0 shadow" }, /* @__PURE__ */ import_react16.default.createElement("div", { className: "text-gray-400 bg-gray-100  overflow-hidden" }, isDevelopment || true ? /* @__PURE__ */ import_react16.default.createElement("div", null, /* @__PURE__ */ import_react16.default.createElement(
    "button",
    {
    ...
    "Documentation"
  )) : /* @__PURE__ */ import_react16.default.createElement(import_react16.default.Fragment, null), /* @__PURE__ */ import_react16.default.createElement("div", { className: "bg-white" }, /* @__PURE__ */ import_react16.default.createElement(
    "a",
    {
      target: "_blank",
    ...
    /* @__PURE__ */ import_react16.default.createElement("div", { className: "px-4 text-lg font-medium w-full" }, "Empirica"),
    /* @__PURE__ */ import_react16.default.createElement("div", { className: "w-14 h-14 p-2 flex-shrink-0 bg-white" }, /* @__PURE__ */ import_react16.default.createElement(Logo, null))
  ))));
}
