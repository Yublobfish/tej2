...
var Scopes = class {
  ...
  update(scope, removed) {
    ...
    const subj = new import_rxjs3.BehaviorSubject(obj);
...
var Step = class {
  constructor(step, ticker) {
    this.running = false;
    this.ticker = new import_rxjs4.BehaviorSubject(void 0);
    this.startAt = 0;
    this.endAt = 0;
    ticker.pipe((0, import_rxjs4.map)(this.recalc.bind(this))).subscribe({
      next: (val) => {
...
var Steps = class {
  constructor(stepsObs, donesObs) {
    ...
    this.ticker = new import_rxjs4.BehaviorSubject(Math.floor(pnow()));
...
function EmpiricaClassic(participantID, provider) {
  const attributesDones = new import_rxjs5.Subject();
  const scopesDones = new import_rxjs5.Subject();
  ...
  const ret = {
    game: new import_rxjs5.BehaviorSubject(void 0),
    player: new import_rxjs5.BehaviorSubject(void 0),
    players: new import_rxjs5.BehaviorSubject([]),
    round: new import_rxjs5.BehaviorSubject(void 0),
    stage: new import_rxjs5.BehaviorSubject(void 0),
    globals: glob.self
  };
