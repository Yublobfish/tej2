...
var Stage = class extends GameOwned {
  get round() {
    ...
    this.addTransitions([
      {
        from: import_tajriba.State.Running,
        to: import_tajriba.State.Ended,
  ...
  return function(_) {
    ...
    _.unique.after(
      ...
      (ctx, { stage, start }) => {
        ...
        ctx.addTransitions([
          {
            from: import_tajriba2.State.Created,
            to: import_tajriba2.State.Running,
            nodeID: timerID,
            cause: "stage start"
          }
        ]);
    ...
    _.after(
      ...
      (ctx, { playerStage, submit }) => {
        ...
        if (haveAllPlayersSubmitted) {
          ctx.addTransitions([
            {
              from: import_tajriba2.State.Running,
              to: import_tajriba2.State.Ended,
    ...
    _.on(
      ...
      (_2, { step, transition: { from, to } }) => {
        ...
        if (from === import_tajriba2.State.Running && to === import_tajriba2.State.Ended) {
...
var Conn = class {
  ...
  async *filteredScopes(filter, cnstrctr) {
    ...
    while (true) {
      ...
      for (const edge of (scopes == null ? void 0 : scopes.edges) ?? []) {
        yield new cnstrctr(this, edge.node);
      }
      if (!(scopes == null ? void 0 : scopes.pageInfo.hasNextPage)) {
        break;
      } else {
...
async function connect(tajURL, token, srtoken, clientName = "api") {
  const tajriba = await import_tajriba3.Tajriba.createAndAwait(tajURL);
  if (!token) {
...
function Lobby(_ = {}) {
  return function(_2) {
    _2.on("player", "introDone", async (ctx, { player }) => {
      ...
      debug("lobby: lobby intro done", (_a = game.batch) == null ? void 0 : _a.inspect());
    ...
    _2.on(
      ...
      (ctx, { step, transition: { from, to } }) => {
        ...
        if (from !== import_tajriba4.State.Running || to !== import_tajriba4.State.Ended) {
          return;
        }
...
async function setupIndividualLobbyTimeout(ctx, game, player) {
  ...
  ctx.addTransitions([
    {
      from: import_tajriba4.State.Created,
      to: import_tajriba4.State.Running,
      nodeID: stepID,
...
async function getTimer(ctx, duration) {
  ...
  try {
    ...
    return (_a = steps[0]) == null ? void 0 : _a.id;
  } catch (err) {
    return;
  }
}
